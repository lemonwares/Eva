  ...
  return (
    <>
      <AdminLayout title="Reviews" showSearch={false}>
        {" "}
        <div className="flex flex-col lg:flex-row gap-6 h-auto lg:h-[calc(100dvh-180px)]">
                  ...
                  return (
                    ...
                    >
                      <div className="flex gap-3">
                        ...
                        >
                          {getInitials(review.author?.name ?? "Anonymous")}
                        </div>
                  ...
                  <div>
                    ...
                    <p className={textSecondary}>
                      ...
                      <span className={`font-medium ${textPrimary}`}>
                        {selectedReview.provider?.businessName || "Unknown"}
                      </span>
                    </p>
                    <p className={`text-sm ${textMuted}`}>
                      {selectedReview.author?.email || "No email"}
                    </p>
                  </div>
                  <p className={`text-sm ${textMuted}`}>
              ...
              >
                ...
                <button
                  className={`p-3 rounded-lg border ${
                    darkMode ? "border-white/10" : "border-gray-200"
                  } ${textMuted} hover:bg-amber-500/10 hover:text-amber-400 hover:border-amber-500/30 transition-colors disabled:opacity-50`}
                  disabled={flagging}
                  onClick={async () => {
                    setFlagging(true);
                    try {
                      ...
                      logger.error("Error flagging review:", err);
                    } finally {
                      setFlagging(false);
                    }
                  }}
      ...
      <ConfirmDeleteModal
        ...
        message={
          selectedReview
            ? `Are you sure you want to reject this review by ${selectedReview.author?.name || "Anonymous"}? This action will remove it from public view.`
            : ""
        }
        confirmText="Reject"
        variant="warning"

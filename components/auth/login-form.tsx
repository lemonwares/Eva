...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Step 2: Post credentials directly to NextAuth — browser handles cookies natively
      ...
      if (!callbackUrl) {
        try {
          ...
          if (meRes.ok) {
            ...
            else if (user?.role === "PROFESSIONAL") destination = "/vendor";
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-5">
        <div className="space-y-1.5">
          <label className="text-sm font-bold text-[#1e2433] ml-1">Email</label>
          <input
            type="email"
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between ml-1">
            <label className="text-sm font-bold text-[#1e2433]">Password</label>
            <Link href="/auth/forgot-password" className="text-xs font-bold text-[#0097b2] hover:underline">
              Forgot password?
            </Link>
          </div>

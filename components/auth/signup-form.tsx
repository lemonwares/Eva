...
export default function SignUpForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeParam === "PROFESSIONAL" || accountType === "PROFESSIONAL") {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="text-center mb-6">
        ...
        {/* Account type toggle — moved to top */}
        <div className="flex items-center justify-center gap-1 p-1 rounded-xl bg-[#f3f4f6] mb-5">
          <button
            type="button"
            onClick={() => {
              setAccountType("CLIENT");
            ...
            className={`flex-1 py-2 px-4 rounded-lg text-sm font-semibold transition-all ${
              !isProfessional
                ? "bg-white text-[#1e2433] shadow-sm"
                : "text-muted-foreground hover:text-[#1e2433]"
            }`}
          >
            I'm a Client
          </button>
      ...
      {/* Google button — disabled for Professional since Google accounts are always CLIENT */}
      {!isProfessional ? (
        <button
          type="button"
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl bg-amber-50 border border-amber-200">
          ...
          <p className="text-xs text-amber-700">Google sign-up creates a client account. Use email to register as a vendor.</p>
        </div>
      )}
